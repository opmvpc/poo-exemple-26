...
PHP 8.4 minimum (`private(set)` et les property hooks). Pas de base de données, pas de serveur web : que du terminal.

## Lecture conseillée, chapitre par chapitre

| Après le chapitre… | Lisez ces fichiers | Ce que vous y retrouvez |
|---|---|---|
| **1 — Objets et classes** | `src/Hero.php`, `src/Dice.php`, `tests/HeroTest.php`, `tests/DiceTest.php` | propriétés typées, promotion de constructeur, `readonly`, `__toString()`, encapsulation (pas de `setHp()`, pas de `hp()` non plus : `public private(set)` et un `set` hook), `isFullHealth` calculé par un `get` hook, fabrique statique `Dice::d6()` |
...
| **5 — Héritage et polymorphisme** | `src/Item.php`, `src/Weapon.php`, `src/Potion.php`, `src/Monster.php`, `src/Goblin.php`, `src/Dragon.php`, `src/InventoryFullException.php` | `abstract`, `extends`, `parent::__construct()`, `public readonly` hérité, `attack()` polymorphe, exception personnalisée |
...
| `src/Hero.php` | La classe `Hero` : l'objet central. **État** (nom, PV, force), **comportement** (`takeDamage`, `heal`, `equip`, `drink`, `attack`), promotion de constructeur, `readonly` sur le nom, `private(set)` sur les PV et l'arme, `__toString()`, et l'**encapsulation** : ni `setHp()` ni `hp()`, la règle des PV vit dans un hook. Elle `use HasHealth` et `implements Fighter` depuis le chapitre 6. | 1, 4, 5, 6 |
...
| `src/Weapon.php` | `Weapon extends Item` : « une épée **est un** objet ramassable ». Ajoute `damage` (`public readonly`) et écrit son `describe()`. | 5 |
...
| `src/HasHealth.php` | Le trait `HasHealth` : les PV en `private(set)` bornés par un `set` hook, `isFullHealth` en `get` hook, et trois méthodes, collés dans `Hero` **et** dans `Monster`. Un trait n'est pas un parent et ne se dessine pas en UML. | 6 |
...
| `tests/DiceTest.php` | Les bornes du lancer (vérifiées 100 fois), les fabriques statiques, et le refus d'un dé à moins de deux faces. |
| `tests/HeroTest.php` | PV au départ, plancher à 0, plafond à `maxHp`, `__toString()`, `name` bien `readonly`, `hp` en `private(set)`, `isFullHealth`, nom vide et `maxHp` nul refusés, un sac par héros, `equip`, `drink`, `attack`. |
| `tests/ItemTest.php` | `Item` abstraite, nom, poids (`readonly`, négatif refusé), rareté par défaut, `value()`, `Stringable`. |
...
## Le style de code, en quatre règles
...
4. **Aucun getter trivial, aucun setter.** Ce qui ne change jamais est
   `public readonly` ; ce que la classe modifie elle-même est `public private(set)`,
   avec un hook si une règle s'applique ; ce qui se calcule est une méthode
   (`totalWeight()`, `value()`) ou un `get` hook (`isFullHealth`).
